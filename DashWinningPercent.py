...
svg_file_path = '/Users/andrewpark/Desktop/Personal/Code/Politics/countychangefull.svg' # County SVG
...
def update_svg_image(button1, button2, button3, correlation_threshold, state_input, county_input, dem_input, rep_input, oth_input):
    ...
    if ctx.triggered_id == 'session-button':
        ...
        margin = [0]*len(length_dem_results)
        ...
        for i in range(length_dem_results): # for EACH of ALL county
            if max(findem_results[i], finrep_results[i], finoth_results[i]) == findem_results[i]:
                margin[i] = min(1,findem_results[i]) # Democrat is +0
            elif max(findem_results[i], finrep_results[i], finoth_results[i]) == finrep_results[i]:
                margin[i] = min(2,finrep_results[i]+1) # Republican is +1
            else:
                margin[i] = min(3,finoth_results[i]+2)  # Other is +2
        
        final_results = pd.DataFrame({"fips": fips, "margin": margin})
        final_results.to_csv("results.csv")
        update_svg(svg_file_path, csv_file_path)  
        with open('/Users/andrewpark/Desktop/Personal/Code/Politics/countychangefull_modified.svg', 'r') as file:
            svg_code = file.read()
    ...
    for i in range(len(demtemp)): # for EACH of ALL county
        ...
        if max(findem_results[i], finrep_results[i], finoth_results[i]) == findem_results[i]:
            margin[i] = min(1,findem_results[i])
        elif max(findem_results[i], finrep_results[i], finoth_results[i]) == finrep_results[i]:
            margin[i] = min(2,finrep_results[i]+1) # gop = +1
        else:
            margin[i] = min(3,finoth_results[i]+2)  # ind = +2
